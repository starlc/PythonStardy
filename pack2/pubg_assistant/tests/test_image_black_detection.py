...
def test_template_matching(template_dir, temp_dir, target_file=None):
    ...
    def check_left_black(img):
        ...
        Returns:
            (is_black, right_border): 左侧是否为黑色以及右边界位置
        """
        if len(img.shape) != 2:
            return False, 0
        ...
        # 检查左侧区域是否接近黑色
        left_region = img[:, :left_width]
        avg_intensity = np.mean(left_region)
        
        # 如果平均亮度小于阈值，认为是黑色区域
        is_black = avg_intensity < 30
        
        # 如果是黑色区域，找到右边界（简化处理，返回1/5宽度）
        right_border = left_width if is_black else 0
        
        return is_black, right_border
    ...
    for temp_file in temp_files:
        ...
        # 检测左侧是否全黑
        try:
            # 尝试使用ImageProcessor中的方法
            is_black, right_border = processor.check_left_black(gray_img)
        except AttributeError:
            # 如果方法不存在，使用我们自己定义的函数
            is_black, right_border = check_left_black(gray_img)
            
        if is_black:
            print(f"检测到左侧全黑，只匹配右侧区域，起始位置: {right_border}")
            gray_img = gray_img[:, right_border:]
            
        # 测试与每个模板的匹配度
        results = []
        for gun_id in gun_list:
            # 检查gun_img_dict[gun_id]的类型和结构
        ...
        # 输出匹配结果（按匹配度排序）
        print(f"{'武器ID':<10} {'匹配度':<10}")
        print("-" * 20)
        for gun_id, similarity in results:
            print(f"{gun_id:<10} {similarity:.2f}")
        
        # 按武器ID数字排序
        numeric_results = sorted(results, key=lambda x: int(x[0]))
        
        print("\n按武器ID排序的结果:")
        print(f"{'武器ID':<10} {'匹配度':<10}")
        print("-" * 20)
        for gun_id, similarity in numeric_results:
            print(f"{gun_id:<10} {similarity:.2f}")
...
if __name__ == "__main__":
    try:
        ...
        if args.file is None:
            args.file = '202505040050570008.png'  # 设置默认值
